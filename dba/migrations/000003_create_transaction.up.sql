...
BEGIN
    ...
    SELECT * FROM account WHERE id = arg_sender_id into temp_sender_account FOR UPDATE;
    IF NOT FOUND THEN
        RAISE EXCEPTION 'sender account missing';
    END IF;
    SELECT * FROM account WHERE id = arg_receiver_id into temp_receiver_account FOR UPDATE;
    IF NOT FOUND THEN
        RAISE EXCEPTION 'receiver account missing';
    END IF;

    -- validate that we have sufficient balance to complete transaction
    SELECT * FROM get_latest_balance(arg_sender_id) INTO most_recent_balance;
    ...
    VALUES (arg_transaction_id, arg_sender_id, arg_receiver_id, arg_type, arg_request_id)
    ...
    VALUES (arg_sender_id, result_transaction.id, arg_amount, arg_request_id);
    ...
    VALUES (arg_sender_id, most_recent_balance.balance, most_recent_balance.hold + arg_amount,
